...
int genmove(char* color,int size,int **board,struct player *wpos ,struct player *bpos, Listptr *history)
{
	...
	do{
		...
		a=rand()%10+1;
		if(!strcasecmp(color,"W") || !strcasecmp(color,"WHITE")){
			if(a<=6 && valid_move(size,color,(wpos->l)-2,wpos->c,board,bpos,wpos)){
				n=(wpos->l)-2;
				m=wpos->c;
			...
			else if(a>6 && a<=8 && valid_move(size,color,wpos->l,(wpos->c)-4,board,bpos,wpos)){
				n=wpos->l;
                	        m=(wpos->c)-4;
			...
			else if(a>8 && a<=10 && valid_move(size,color,wpos->l,(wpos->c)+4,board,bpos,wpos)){
				n=wpos->l;
                	        m=(wpos->c)+4;
				insert_at_start(history,wpos->l,(wpos->c)+4,color,NULL,wpos->l,wpos->c);
				playgenmove(size,color,board,wpos,bpos,history,wpos->l,(wpos->c)+4);
